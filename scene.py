...
class Fibonacci(Scene):
    def construct(self):
        ...
        clearAnim = [FadeOut(definition), FadeOut(cdots), Transform(title, newTitle)]
        for eq in eqs:
            clearAnim += [FadeOut(eq)]
        self.play(*clearAnim)
        self.wait()

        # Clean up are sequence
        modGroup = VGroup(*modEqs)
        self.play(modGroup.animate.arrange(RIGHT).shift(LEFT*2.0), FadeOut(mdots))
        newNums = VGroup(*[MathTex(i) for i in [9, 4, 3, 7]]).arrange(RIGHT).next_to(modGroup, RIGHT)
