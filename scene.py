...
class Fibonacci(Scene):
    def construct(self):
        ...
        eqs = []
        reducedEqs = []
        for i, eq in enumerate(fibLines(9)):
        ...
        cdots = Tex(r"\ldots", font_size=smallFont).next_to(eqs[-1], RIGHT, buff=hD*2/3).align_to(p0, UP).shift(DOWN * 0.25) # This is also not correctly aligned
        eqs = [p0, p1] + eqs
        self.play(
            AnimationGroup(
